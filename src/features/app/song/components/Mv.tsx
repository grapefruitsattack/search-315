...
export default function Mv({ mvInfos }: { mvInfos: MvInfo[] }) {
    ...
    const displayMv: MvInfo[] = youtubeMv.toReversed().concat(otherMv.toReversed());
    ...
    <section className={`${isOpen?'pt-1':'hidden'}`}>
      ...
      {displayMv.length<=1
        ?(
          ...
          `}>
            <VideoBlock videoId={displayMv[0].videoId}/>
          </div>
        )
        :(
          <VideoCarousel videoIdArray={displayMv.map(data=>data.videoId)}/>
        )
      }
