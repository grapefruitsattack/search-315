...
export default async function StoryWithSong({ songId }: { songId: string }) {
  const storyIds = relation_story_other.filter(data=>data.songId===songId).map(data=>data.storyId);
