...
export default function UnitPage({ singingInfoId }: { singingInfoId: string  }) {
  const id = singingInfoId;
  ...
  const [isLiveOpen, setIsLiveOpen] = useState(true);
    ...
    return (
    <>
        <title>{ `${idolName} ${'\u00a0'}|${'\u00a0\u00a0'}サーチサイコー`}</title>
        <meta name="description" content={`「${idolName}」の楽曲情報・サブスク配信状況をチェック！ |  サーチサイコー`}/>
    ...
    <article className=" pb-96 px-2 mobileS:px-12 lg:px-24 bg-white lg:max-w-[1500px] lg:m-auto font-sans">
      <div className={`
      relative 
