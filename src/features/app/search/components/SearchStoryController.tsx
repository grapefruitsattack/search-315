          ...
          <div className={`flex mt-1 px-8 gap-2 justify-center`}>
            <div className={`relative flex w-[600px] p-1 bg-slate-900 rounded-full`}>
              <span className={`absolute inset-0 m-1 pointer-events-none`}>
                <span 
                  className={`
                    absolute inset-0 w-1/2 bg-teal-500 rounded-full shadow-sm shadow-indigo-950/10 transform transition-transform duration-150 ease-in-out 
                  ${values["andor"]==="or"?'translate-x-0':'translate-x-full'}`}
                >
                </span>
              </span>
              <button 
              ...
              <button 
                className={`
                  relative flex-1 text-sm font-medium h-6 rounded-full focus-visible:outline-none focus-visible:ring focus-visible:ring-indigo-300 transition-colors duration-150 ease-in-out 
          ...
          <div className='flex mt-1 px-8 gap-2 justify-center'>
            ...
            <Tooltip hasArrow label='1曲以上該当するよう条件を設定してください' bg='red.600' isOpen = {tooltipOn}>
              <button 
                className='rounded-lg
                  text-white text-sm font-bold leading-tight
                  hover:bg-green-500/70 bg-green-500
                  transition-all duration-500 ease-out
                  w-[200px] p-2'
                onClick={() => {
                  if(errorCheck()){
